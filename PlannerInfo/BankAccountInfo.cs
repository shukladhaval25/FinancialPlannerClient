...
namespace FinancialPlannerClient.PlannerInfo
{
    ...
    {
        const string GET_ALL_BankAccount_API = "BankAccount/GetAll?clientId={0}";
        const string GET_ALL_BY_ID_API = "BankAccount/GetById?id={0}&clientId={1}";
        const string ADD_BankAccount_API = "BankAccount/Add";
        const string UPDATE_BankAccount_API = "BankAccount/Update";
        const string DELETE_BankAccount_API = "BankAccount/Delete";
        DataTable _dtBankAccount;
        internal IList<BankAccountDetail> GetAll(int clientId)
        {
            IList<BankAccountDetail> BankAccountObj = new List<BankAccountDetail>();
            try
            {
                FinancialPlanner.Common.JSONSerialization jsonSerialization = new FinancialPlanner.Common.JSONSerialization();
                string apiurl = Program.WebServiceUrl +"/"+ string.Format(GET_ALL_BankAccount_API,clientId);
                ...
                {
                    BankAccountObj = jsonSerialization.DeserializeFromString<IList<BankAccountDetail>>(restResult.ToString());
                }
